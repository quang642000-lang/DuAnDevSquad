    ...
    <div class="main-content">
        ...
        <header class="top-navbar" style="height: auto; min-height: 70px; padding: 15px 24px; flex-wrap: wrap; gap: 15px;">
            ...
            <div class="d-flex align-items-center ms-auto">
                <button class="btn btn-success btn-sm rounded-pill px-3 fw-bold shadow-sm me-3" onclick="exportProfessionalCSV()">
                    <i class="bi bi-file-earmark-excel-fill me-1"></i> Xuất Báo Cáo
                </button>
                <div class="dropdown">
...
<script>
    ...
    function exportProfessionalCSV() {
        ...
        if (rows.length === 0 || (rows.length === 1 && rows[0].innerText.includes("Chưa có giao dịch"))) {
            showToast("Cảnh báo: Không có dữ liệu giao dịch nào để xuất báo cáo!", "warning");
            return;
        }

        let tuNgay = document.getElementById('filterTuNgay').value;
        let denNgay = document.getElementById('filterDenNgay').value;
        let maNVSelect = document.getElementById('filterMaNV');
        let tenNVDangLoc = maNVSelect.options[maNVSelect.selectedIndex].text;

        let dateObj = new Date();
        let reportTimestamp = dateObj.toLocaleString('vi-VN');

        let csvContent = [];
        csvContent.push("HỆ THỐNG QUẢN LÝ BÁN HÀNG TEA POS");
        csvContent.push("BÁO CÁO DOANH THU CHI TIẾT");
        csvContent.push("");

        csvContent.push(`Thời gian trích xuất:,${reportTimestamp}`);
        csvContent.push(`Kỳ báo cáo từ ngày:,${tuNgay || "Đầu kỳ"}`);
        csvContent.push(`Đến ngày:,${denNgay || "Hôm nay"}`);
        csvContent.push(`Bộ lọc Nhân viên:,${tenNVDangLoc}`);
        csvContent.push("");

        let totalDoanhThuText = document.getElementById('statDoanhThu').innerText.replace(/\./g, '').replace('₫', '').trim();
        let totalDonHangText = document.getElementById('statDonHang').innerText.replace('đơn', '').trim();

        csvContent.push(`TỔNG DOANH THU LỌC:,"${totalDoanhThuText} VNĐ"`);
        csvContent.push(`TỔNG SỐ ĐƠN HÀNG:,"${totalDonHangText} Đơn"`);
        csvContent.push("");

        csvContent.push("STT,GIỜ TẠO,MÃ HÓA ĐƠN,NGƯỜI THU TIỀN,DOANH THU ĐƠN (VNĐ),TRẠNG THÁI");

        rows.forEach((row, index) => {
            let cols = row.querySelectorAll("td");
            let gioTao = cols[0].innerText.trim();
            let maHD = cols[1].innerText.trim();
            let nguoiThu = cols[2].innerText.trim().replace(/\n/g, ' - ');
            let doanhThu = cols[3].innerText.replace(/\./g, '').replace('₫', '').trim();
            let trangThai = cols[4].innerText.trim();

            csvContent.push(`"${index + 1}","${gioTao}","${maHD}","${nguoiThu}","${doanhThu}","${trangThai}"`);
        });

        let filename = `Bao_Cao_Doanh_Thu_TeaPOS_${dateObj.getFullYear()}${dateObj.getMonth()+1}${dateObj.getDate()}.csv`;
